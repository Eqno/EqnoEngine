...
Validation::Validation()
    : enabled(VulkanConfig::ENABLE_VALIDATION_LAYER),
...
Validation::Validation(const bool enabled, CStrings layers,
...
VkResult Validation::CreateMessengerEXT(
    const VkInstance& instance, const VkAllocationCallbacks* pAllocator,
  ...
  if (const auto func = reinterpret_cast<PFN_vkCreateDebugUtilsMessengerEXT>(
    ...
    return func(instance, &messengerCreateInfo, pAllocator, pDebugMessenger);
  }
...
void Validation::DestroyMessenger(const VkInstance& instance) const {
  if (enabled) {
    DestroyMessengerEXT(instance, debugMessenger, nullptr);
  }
}
