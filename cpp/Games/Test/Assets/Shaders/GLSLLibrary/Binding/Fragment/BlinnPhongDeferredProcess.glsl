...
layout(input_attachment_index = 0, binding = 2) uniform subpassInputMS inColor;
layout(input_attachment_index = 1, binding = 3) uniform subpassInputMS inNormal;
layout(input_attachment_index = 2, binding = 4) uniform subpassInputMS inPosition;
layout(input_attachment_index = 3, binding = 5) uniform subpassInputMS inMaterial;
layout(input_attachment_index = 4, binding = 6) uniform subpassInputMS inCameraNormal;
layout(input_attachment_index = 5, binding = 7) uniform subpassInputMS inCameraPosition;
#else
layout(input_attachment_index = 0, binding = 2) uniform subpassInput inColor;
layout(input_attachment_index = 1, binding = 3) uniform subpassInput inNormal;
layout(input_attachment_index = 2, binding = 4) uniform subpassInput inPosition;
layout(input_attachment_index = 3, binding = 5) uniform subpassInput inMaterial;
layout(input_attachment_index = 4, binding = 6) uniform subpassInput inCameraNormal;
layout(input_attachment_index = 5, binding = 7) uniform subpassInput inCameraPosition;
#endif

#ifdef EnableShadowMap
layout(binding = 8) uniform sampler2DShadow shadowMapSamplers[MaxLightNum];
#endif
...
for (int i=0; i<EnableMultiSample; i++) { \
    ...
    fragPosition += subpassLoad(inPosition, i); \
...
vec4 fragPosition = subpassLoad(inPosition); \
